...
2. **Icône barre système** — pas implémentée. Nécessaire pour que l'admin voie que PrinterBridge tourne (choix produit assumé, cf. Positionnement), et préalable à un vrai lancement "un clic" une fois qu'on voudra que le raccourci démarre l'unité `systemd` plutôt que le binaire brut (cf. `packaging/linux/TESTING.md`, dernière section).
3. **Reprise automatique sur crash — Linux fait, Windows/Mac pas commencés.** L'unité `systemd --user` existe (voir § Positionnement + `packaging/`) mais n'a jamais tourné sur une vraie machine. L'équivalent Windows (Recovery d'un Service) et macOS (`launchd KeepAlive`) reste à faire — et se heurtera probablement à la même question que sur Linux (comment démarrer "à la main" un service tout en restant supervisé par l'OS).
4. **Compatibilité de contrat d'API entre les deux repos** — PluriBourse et PrinterBridge évoluent maintenant séparément ; proposition : versionnage dans l'URL (`/v1/printers`, etc.), à valider et instrumenter (tests de contrat) quand un premier changement cassant se présentera.
5. **Découverte imprimante WiFi thermique** — toujours reportée. Contrairement au réseau/A4 (résolu via `javax.print`), une imprimante thermique WiFi utilise typiquement un protocole ESC/POS brut sur socket (pas le spouleur OS) — nécessiterait sa propre couche transport (proche de l'actuel Bluetooth mais en TCP) et le découplage contenu/transport côté PluriBourse déjà noté plus haut.
